...
changes (see the options for the =(bespoke-set-mode-line)= variable. You can set
italics for keywords or for comments (or both). You can set variable-pitch for
headlines using =(bespoke-set-variable-pitch)=. There is a toggle function for toggling
...
bespoke-set-visual-bell t)= in your setup of the theme. All options are available via
