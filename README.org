...
There is an optional custom mode line, and may be set as a header line (as in
nano-emacs) or as the normal footer line, or the theme can make marely cosmetic
changes (see the options for the =(set-bespoke-mode-line)= variable. There is a toggle
function for toggling between light and dark theme variants: =(bespoke/toggle-theme)=.
Also included is a useful footer line visual alert "bell" inspired by [[https://github.com/hlissner/doom-emacs][DOOM emacs]]. To
use set =(setq set-bespoke-visual-bell t)= in your setup of the theme.
