Custom medium contrast light and dark themes for Emacs (intended for GUI, though
...
cursor colors. I'm trying to add faces on a regular basis. If there is a face you'd
like to see added please open an issue. 

*NOTE*: It is early days for this theme, so there may be fairly significant changes
made in various commits. I'm continuing to tweak colors slightly to find the best
balance between aesthetics and readability. Suggestions and pull requests are welcome!
...
- You can set the size of the mode line with =(bespoke-set-modeline-size)=
